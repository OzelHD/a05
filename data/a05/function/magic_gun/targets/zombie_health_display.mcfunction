...
# Remove old displays if too many (cleanup)
execute as @e[tag=zombie_health,type=minecraft:text_display] unless entity @e[tag=magic_gun_target,distance=..1,type=minecraft:zombie] run kill @s

# For each zombie, summon or update a text_display
execute as @e[tag=magic_gun_target,type=minecraft:zombie] at @s unless entity @e[tag=zombie_health,distance=..2.4,type=minecraft:text_display] run summon minecraft:text_display ~ ~2.3 ~ {Tags:["zombie_health"],billboard:"center",background:0,see_through:true,shadow:true,transformation:{scale:[0.7f,0.7f,0.7f],translation:[0.0f,0.0f,0.0f],left_rotation:[1.0f,0.0f,0.0f,0.0f],right_rotation:[1.0f,0.0f,0.0f,0.0f]}}
